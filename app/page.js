...
function FBConnect({ onConnect, onSkip }) {
  ...
  const handleConnect = () => {
    ...
    onConnect({ id: "102907851890377", token: token.trim(), name: "Thumb Forecast", followers: 13514 });
  };
  return (
...
export default function App() {
  ...
  const [fbCreds, setFbCreds] = useState(() => {
    ...
    const envToken = process.env.NEXT_PUBLIC_FB_TOKEN;
    if (envToken) {
      return { id: "102907851890377", token: envToken, name: "Thumb Forecast", followers: 13514 };
    }
  ...
  useEffect(() => {
    ...
    const fetchFB = async () => {
      setFbLoading(true);
      try {
        const url = `/api/facebook?pageId=${encodeURIComponent(fbCreds.id)}&token=${encodeURIComponent(fbCreds.token)}`;
        const r = await fetch(url);
        const json = await r.json();
        console.log("FB response:", JSON.stringify(json).slice(0, 200));
        if (json.fan_count) {
          setFbCreds(prev => ({ ...prev, followers: json.fan_count, name: json.name || prev.name }));
          setD(prev => ({ ...prev, facebook: { ...prev.facebook,
            reach: json.talking_about_count || prev.facebook.reach,
            pageViews: json.talking_about_count || prev.facebook.pageViews,
            posts: (json.posts?.data || []).filter(p =>
